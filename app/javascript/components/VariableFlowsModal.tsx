...
// #variable_transactions), so same-month rows are already contiguous: bucket them into
// month groups preserving that order and sum each month's subtotal.
...
// Drill-through receipt for the forecast's Variable Einnahmen/Ausgaben · Ø N Mt. ledger
// rows: the individual non-recurring transactions that make up the average
// (#variable_transactions), footer = the Σ ÷ N Mt. = Ø till math that reconciles to the row.
export default function VariableFlowsModal({ kind, scope, locale, t, onClose }: {
  ...
  useEffect(() => {
    let alive = true
    const url = `/api/v1/statistics/variable_transactions?${withScope(new URLSearchParams({ kind }), scope).toString()}`
    api(url)
      .then(async res => {
  ...
  const subtitle = data ? t('statistics.forecast.variable_modal.subtitle', { n: data.months }) : undefined

  // Footer = the Σ ÷ N Mt. = Ø average tally the forecast row sits behind.
  const footer: ReactNode = data && data.transactions.length > 0 ? (
